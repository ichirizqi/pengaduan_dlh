                        ...
                        <div class="col-xl-12 col-lg-7">
                            <div class="card shadow mb-4">
                                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                                    <h6 class="m-0 font-weight-bold text-black">Form Edit Status Pengaduan</h6>
                                </div>
                                <!-- Card Body -->
                                <div class="card-body">
                                    <form @submit.prevent="update">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" v-model="post.nama" placeholder="Masukkan nama" required>
                                        </div>
                                        <div v-if="validation.nama" class="mt-2 alert alert-danger">
                                                {{ validation.nama[0] }}
                                        </div>
                                        <button type="submit" class="btn btn-success">Simpan</button>
                                        <a href="/admin/status" class="btn btn-default border-secondary pull-right ml-2 hover:bg-gray-200 hover:text-white">Kembali</a>
                                    </form>
                                </div>
...
export default {
    ...
    setup() {
        ...
        onMounted(() => {
            ...
            axios.get(`http://localhost:8000/api/admin/statuspengaduan/${route.params.id}`)
            .then(response => {
              
              //assign state posts with response data
              post.nama    = response.data.data.nama
            //   post.password  = response.data.data.password  
        ...
        function update() {
            ...
            axios.post(`http://localhost:8000/api/admin/statuspengaduan/${route.params.id}`, {
                nama: nama
            }).then(() => {
                router.push({
                    name: 'admin-status'
